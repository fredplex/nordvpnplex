...
**Decision**: `scripts/bump.sh` now (1) refuses to proceed if `Dockerfile`/`README.md`/`CLAUDE.md` still contain conflict markers, and (2) auto-appends a one-line Changelog entry to `README.md` on every run (originally with a `<!-- TODO -->` placeholder; since the 2026-07-09 pipeline review it writes a real summary, and since 2026-07-10 an optional third argument sets the wording for feature/fix bumps). Both bump workflows now run a `gh pr list` guard for any open `auto/*`-branch PR before applying a bump or opening a new one, skipping (with a logged reason) if one already exists.
...
**Consequences**: A `task bump` run will now hard-fail with a clear error if it encounters leftover conflict markers, rather than editing around them. Every bump PR includes an auto-written Changelog line — image-only bumps default to "Base image refresh" wording, so feature/fix bumps should pass the third `bump.sh` argument (see `docs/build-and-publish.md` §5). If a bump PR sits open for a long time, the *other* automation logs a skip on its own runs until that PR is merged or closed.
