<!-- prime: version=3.0.8 template=.ai/workflows/session-close.md date=2026-07-01 -->
...
8. **Verify documentation sync**
   ...
   - **Active AGENTS.md scan — mandatory before closing**: Check `AGENTS.md` for staleness in two passes:
     - **Numeric claims**: grep for every count, number, or version in prose (file counts, test counts, version numbers). For each, verify it still matches actual project state — compare file/module counts against the real directory structure, test counts against `task verify` output, version numbers against project manifests.
