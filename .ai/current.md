...
**Status**: Container and base image version logs added to startup. Custom log version banner is now populated using s6 environment loading, and the base image version/digest is written to `/build_version` during image compilation to display inside LSIO's standard startup branding block.
...
- Working tree: clean of task work. 3 commits on the task branch.
