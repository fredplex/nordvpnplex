...
- **Never bump the base image** (`ghcr.io/linuxserver/baseimage-ubuntu:noble`) without explicit instruction. The base is digest-pinned (`ARG BASE_DIGEST` in the [Dockerfile](Dockerfile)) — changing the digest is the same as bumping the base.
